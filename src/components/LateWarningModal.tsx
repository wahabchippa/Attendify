        ...
        <div className="p-6">
          ...
          <div className="mb-4 grid grid-cols-2 gap-2">
            {REASONS.map(r => (
              <button
                ...
                className={`rounded-md border p-3 text-xs font-bold transition-all ${
                  selected === r.label
          ...
          {selected === 'Other' && (
            <textarea
              ...
              rows={2}
              className="mb-4 w-full rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:border-amber-500 focus:outline-none"
            />
          )}
          <div className="flex gap-3">
            <button onClick={onSkip} className="flex-1 rounded bg-slate-100 py-3 text-sm font-bold text-slate-600">
              Skip
            </button>
            <button
