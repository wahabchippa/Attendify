...
export default function Dashboard({ currentUser, onLogout }: DashboardProps) {
 ...
 const [pendingCheckIn, setPendingCheckIn] = useState<AttendanceRecord | null>(null);
 ...
 const pendingCheckInRef = useRef<AttendanceRecord | null>(null);
 ...
 // ✅ loadTodayData — sync se overwrite protection
 const loadTodayData = () => {
 ...
 const found = active || today || null;

 // ✅ IMPORTANT: Agar already checked in hai aur sync se null aaya, to overwrite MAT karo
 setTodayRecord(prev => {
 if (prev && prev.checkIn && !found) {
 // Record abhi Supabase mein sync nahi hua — local state protect karo
 return prev;
 }
 return found;
 });
 ...
 // ✅ Main sync effect — sirf currentUser.id pe depend karta hai
 useEffect(() => {
 checkedInTodayRef.current = false;
 ...
 // ✅ Agar late modal open hai ya check-in blocked hai to skip karo
 if (lateModalOpenRef.current || syncBlockedRef.current) return;
 await syncAll();
 loadTodayData();
 }, 30000); // ✅ 30 seconds — overwrite ka risk kam
 ...
 pendingCheckInRef.current = record;
 // ✅ Sync block karo jab tak modal complete na ho
 syncBlockedRef.current = true;
 lateModalOpenRef.current = true;
 ...
 const record = pendingCheckInRef.current;
 if (!record) {
 lateModalOpenRef.current = false;
 syncBlockedRef.current = false;
 setShowLateModal(false);
 showNotif('error', 'Something went wrong. Please try again.');
 return;
 }
 try {
 const recordWithReason: AttendanceRecord = {
 ...record,
 ...
 lateModalOpenRef.current = false;
 // ✅ 3 second baad sync unblock karo — Supabase ko time do
 setTimeout(() => { syncBlockedRef.current = false; }, 3000);
 showNotif('warning', `⚠️ Late check-in recorded. Reason: ${reason}`);
 } catch (err) {
 console.error('Late check-in error:', err);
 lateModalOpenRef.current = false;
 syncBlockedRef.current = false;
 setShowLateModal(false);
 setPendingCheckIn(null);
 pendingCheckInRef.current = null;
 showNotif('error', 'Check-in failed. Please try again.');
 }
 };
 ...
 pendingCheckInRef.current = null;
 lateModalOpenRef.current = false;
 // ✅ 3 second baad sync unblock karo
 setTimeout(() => { syncBlockedRef.current = false; }, 3000);
 showNotif('warning', '⚠️ Late check-in recorded (no reason given)');
 } catch (err) {
 console.error('Late skip error:', err);
 lateModalOpenRef.current = false;
 syncBlockedRef.current = false;
 setShowLateModal(false);
 setPendingCheckIn(null);
 ...
 <button onClick={() => { setShowWFHModal(false); setWfhReason(''); }} className="flex-1 py-3 rounded border border-slate-200 text-slate-600 text-sm font-bold hover:bg-slate-50 transition-all active:scale-[0.97]">
 Cancel
 </button>
 <button onClick={handleWFHRequest} className="flex-1 py-3 rounded bg-gradient-to-r from-slate-900 to-slate-800 text-white text-sm font-bold shadow-lg shadow-slate-900/10 hover:shadow-blue-500/30 transition-all active:scale-[0.97]">
 Submit Request
 </button>
 </div>
 </div>
 ...
 <button onClick={() => setShowQuitConfirm(false)} className="flex-1 py-3 rounded border border-slate-200 text-slate-600 text-sm font-bold hover:bg-slate-50 transition-all active:scale-[0.97]">Stay</button>
 <button onClick={onLogout} className="flex-1 py-3 rounded bg-red-600 hover:bg-red-700 text-white text-sm font-bold shadow-lg shadow-red-600/20 transition-all active:scale-[0.97]">Quit</button>
 </div>
 </div>
 </div>,
 document.body
 ...
 <button onClick={() => setShowWarningModal(false)} className="w-full py-3.5 rounded bg-red-600 hover:bg-red-700 active:scale-[0.97] text-white font-bold text-sm transition-all shadow-lg shadow-red-600/20">
 I Understand
 </button>
 </div>
 </div>,
 ...
 {/* MAIN LAYOUT */}
 <div className={`max-w-5xl mx-auto p-4 md:p-6 space-y-3 transition-all duration-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
 ...
 <div className="w-12 h-12 bg-white/15 rounded-md flex items-center justify-center text-sm font-bold border border-white/20">
 ...
 <button onClick={() => setShowQuitConfirm(true)} className="w-10 h-10 bg-white/10 hover:bg-white/20 rounded flex items-center justify-center border border-white/10 transition-all active:scale-[0.97]" aria-label="Quit">
 ...
 <button onClick={() => handleApproveAccount(req, 'employee')} className="px-3.5 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded text-xs font-bold transition-all active:scale-[0.97]">✓ Employee</button>
 <button onClick={() => handleApproveAccount(req, 'admin')} className="px-3.5 py-2 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded text-xs font-bold transition-all active:scale-[0.97]">✓ Admin</button>
 <button onClick={() => handleApproveAccount(req, 'manager')} className="px-3.5 py-2 bg-slate-100 hover:bg-blue-200 text-slate-900 rounded text-xs font-bold transition-all active:scale-[0.97]">✓ Manager</button>
 <button onClick={() => handleRejectAccount(req)} className="px-3.5 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded text-xs font-bold transition-all active:scale-[0.97] ml-auto">✕ Reject</button>
 </div>
 </div>
 ))}
 </div>
 ...
 officeLocation === null && !gpsOff ? 'bg-slate-400 animate-pulse'
 : displayLabel.includes('Office') ? 'bg-emerald-500'
 : displayLabel === '📍 Location Off' ? 'bg-amber-500'
 : 'bg-red-500'
 }`} />
 ...
 className="w-full py-4 rounded-md bg-gradient-to-r from-slate-900 to-slate-800 hover:from-[#1d4ed8] hover:to-[#3b82f6] disabled:from-slate-300 disabled:to-slate-400 text-white font-bold text-base transition-all shadow-lg shadow-blue-600/25 hover:shadow-blue-500/35 active:scale-[0.97] disabled:shadow-none">
 {checkingIn ? (
 ...
 {!todayWFHRequest && (
 <button onClick={() => setShowWFHModal(true)} className="w-full py-3.5 rounded-md border-2 border-dashed border-slate-200 text-slate-800 font-bold hover:bg-slate-50 text-sm transition-all active:scale-[0.97]">
 ...
 <button onClick={() => handleCheckOut()} className="w-full py-4 rounded-md bg-slate-800 hover:bg-slate-900 active:scale-[0.97] text-white font-bold text-base flex items-center justify-center gap-2 shadow-lg shadow-slate-800/20 transition-all">
 ...
 {/* 🆕 Manager ke liye All Offices Distance */}
 <div className="mt-4">
 <AllOfficesDistance />
 </div>
 </div>
 ...
 <button onClick={() => handleApproveWFH(req)} className="px-4 py-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded text-xs font-bold transition-all active:scale-[0.97]">✓ Approve</button>
 <button onClick={() => handleRejectWFH(req)} className="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded text-xs font-bold transition-all active:scale-[0.97]">✕ Reject</button>
 </div>
 </div>
 );
 })}
 ...
 <button onClick={() => setShowQuitConfirm(true)} className="w-full py-3.5 rounded-md border border-red-200 text-red-500 hover:bg-red-50 font-bold text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.97]">
 Quit Attendify
 </button>
 </div>
