...
const ACTION_CONFIG: Record<string, { label: string; icon: string; color: string; bg: string; border: string }> = {
  ...
  check_out:         { label: 'Check Out',          icon: '←',  color: 'text-slate-900',    bg: 'bg-slate-50',    border: 'border-slate-200' },
  ...
  device_bind:       { label: 'Device Bound',       icon: '📱', color: 'text-slate-900',    bg: 'bg-slate-50',    border: 'border-slate-200' },
  ...
  timing_change:     { label: 'Timing Updated',     icon: '⏰', color: 'text-slate-900',    bg: 'bg-slate-50',    border: 'border-slate-200' },
...
const SEVERITY_CONFIG = {
  info:     { label: 'Info',     color: 'text-slate-800',   bg: 'bg-slate-50',   border: 'border-slate-200' },
  ...
  return (
    <div className={`space-y-5 font-sans transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
      ...
      <div className="bg-gradient-to-r from-[#0f172a] via-[#1e293b] to-[#0f172a] rounded-lg p-5 text-white relative overflow-hidden shadow-md">
        ...
        <div className="absolute -bottom-8 -left-8 w-28 h-28 bg-slate-500/10 rounded-full blur-lg" />
        <div className="relative z-10 flex items-center justify-between">
          ...
          <button
            ...
            disabled={syncing}
            className="flex items-center gap-2 px-4 py-2.5 bg-white/10 hover:bg-white/20 rounded-md text-white text-xs font-bold border border-white/10 transition-all active:scale-95 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total Logs',    value: stats.total,    color: 'text-slate-900', bg: 'from-blue-50 to-indigo-50',     border: 'border-slate-200',   icon: '📋' },
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-sm p-5 space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-md pl-10 pr-4 py-3 text-sm font-medium focus:outline-none focus:border-slate-900 focus:ring-2 focus:ring-slate-900/20 transition-all placeholder-slate-400"
          />
        </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold rounded transition-all capitalize ${
                  severityFilter === s
                    ...
                    : s === 'info'     ? 'bg-slate-800 text-white shadow-md'
                    : 'bg-gradient-to-r from-slate-900 to-slate-800 text-white shadow-md'
                    : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                ...
                >
                  ...
                  >
                    <div className="flex items-start gap-3">
                      {/* Action Icon */}
                      <div className={`w-9 h-9 ${cfg.bg} rounded flex items-center justify-center text-sm border ${cfg.border} shrink-0 mt-0.5`}>
                        {cfg.icon}
                      </div>
