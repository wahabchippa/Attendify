  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)}
        className="relative w-9 h-9 flex items-center justify-center rounded hover:bg-slate-100 transition-all">
        ...
        {unreadCount > 0 && (
          <div className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 rounded-full flex items-center justify-center text-[11px] font-medium text-white border border-white">
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute top-12 right-0 w-80 max-h-[500px] bg-white rounded-lg shadow-lg border border-slate-200 z-[100] overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50/30 flex items-center justify-between">
              ...
              {unreadCount > 0 && (
                <button onClick={handleMarkAllRead} className="text-xs font-bold text-slate-900">Mark all read</button>
              )}
            </div>
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map(n => (
                  <div key={n.id} onClick={() => !n.isRead && handleMarkRead(n.id)}
                    className={`px-4 py-3 border-b border-slate-50 cursor-pointer hover:bg-slate-50 ${!n.isRead ? 'bg-slate-50/30' : ''}`}>
                    <div className="flex items-start gap-2">
                      {!n.isRead && <div className="w-2 h-2 bg-slate-900 rounded-full mt-1.5 shrink-0" />}
