...
import { getEmployees, getAttendanceEmployees, getAttendanceRecords, getLocationFromIP } from '../store';
...
export default function History({ currentUser }: HistoryProps) {
  ...
  const getEmployeeName = (id: string) => getEmployees().find(e => e.id === id)?.name || 'Unknown';
  const getEmployeeAvatar = (id: string) => getEmployees().find(e => e.id === id)?.avatar || '?';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">View and filter records</p>
        </div>
        <button onClick={downloadCSV} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 text-sm font-medium hover:bg-emerald-100 transition-colors">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {[
          ...
          { label: 'Time in Office', value: stats.totalHours, color: 'text-slate-600' },
        ].map(s => (
              ...
              filteredRecords.map(record => (
                ...
                >
                  ...
                  <div className="text-blue-600 text-sm font-mono font-medium">
                    {record.totalHours > 0 ? `${record.totalHours.toFixed(1)}h in office` : '—'}
                  </div>
                  <div className="text-sm">
                    <span className="text-blue-600 text-xs font-medium">{getLocationFromIP(record.ipAddress)}</span>
                  </div>
                </div>
              ))
            )}
