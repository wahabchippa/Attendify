...
export async function addEmployee(emp: Employee) {
  ...
  try { const q = db.from('employees'); if (q) await q.upsert({ id: emp.id, name: emp.name, role: emp.role, pin: emp.pin, avatar: emp.avatar }); } catch {}
}
...
export async function updateEmployeePin(empId: string, newPin: string) {
  ...
  try { const q = db.from('employees'); if (q) await q.update({ pin: newPin }).eq('id', empId); } catch {}
}

export async function removeEmployee(empId: string) {
  cacheSet('c_emp', getEmployees().filter(e => e.id !== empId));
  try { const q = db.from('employees'); if (q) await q.delete().eq('id', empId); } catch {}
}
