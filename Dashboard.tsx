...
export default function Dashboard({ currentUser, onLogout }: DashboardProps) {
  ...
  useEffect(() => {
    if (canMarkAttendance) checkWiFi();
    loadTodayData();
  }, []);
  ...
  const checkWiFi = async () => {
    ...
    setWifiStatus(result.isConnected ? 'connected' : 'disconnected');
    setWifiIP(result.ipAddress);
  };
  ...
  const handleCheckIn = async () => {
    if (wifiStatus !== 'connected') {
      showNotif('error', 'Office WiFi se connect karein pehle!');
      return;
    }
    setCheckingIn(true);
    const result = await verifyWiFiConnection();
    if (!result.isConnected) {
      ...
      showNotif('error', 'WiFi verification failed!');
      return;
    }
    const now = new Date();
  ...
  const handleCheckOut = () => {
    ...
    if (totalHours < settings.minHoursForHalfDay) status = 'half-day';
  ...
  const handleApproveWFH = (request: WFHRequest) => {
    ...
    loadTodayData();
    showNotif('success', `WFH approved for ${EMPLOYEES.find(e => e.id === request.employeeId)?.name}`);
  };
      ...
      {showQuitConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm animate-fade-in text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-800 mb-1">Quit Application?</h3>
            <p className="text-slate-500 text-sm mb-5">You will be logged out of the attendance system.</p>
          ...
          <div className="px-6 py-6">
            <div className="flex flex-col items-center">
              ...
              <div className={`mt-4 flex items-center gap-2 px-4 py-2 rounded-full text-sm ${
                wifiStatus === 'connected' ? 'bg-emerald-50 text-emerald-700' :
                wifiStatus === 'disconnected' ? 'bg-red-50 text-red-600' : 'bg-amber-50 text-amber-600'
              }`}>
                <span className={`w-2 h-2 rounded-full ${
                  wifiStatus === 'connected' ? 'bg-emerald-500' :
                  wifiStatus === 'disconnected' ? 'bg-red-500' : 'bg-amber-500 animate-pulse'
                }`}></span>
                {wifiStatus === 'connected' 
                  ? `Office WiFi Connected • ${wifiIP}` 
              ...
              <div className="mt-6 w-full max-w-xs">
                {!todayRecord ? (
                  <>
                    ...
                    <button onClick={checkWiFi} disabled={wifiStatus === 'checking'}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <p className="text-slate-400 text-sm">{getGreeting()}</p>
              <h1 className="text-lg font-semibold text-slate-800">{currentUser.name}</h1>
              <p className="text-blue-600 text-xs font-medium">Manager Dashboard</p>
            </div>
      ...
      {isAdmin && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            { label: 'Absent', value: ATTENDANCE_EMPLOYEES.length - todayAllRecords.length, color: 'text-red-600', bg: 'bg-red-50' },
            { label: 'Total', value: ATTENDANCE_EMPLOYEES.length, color: 'text-slate-600', bg: 'bg-slate-50' },
          ].map(stat => (
